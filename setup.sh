...
if type apachectl &> /dev/null; then
    cp "$CURRENT_DIR"/wp-cli.yml.example "$WORDPRESS_SETUP_DIR"/wp-cli.yml
    cp "$CURRENT_DIR"/wp-cli.yml.example "$WORDPRESS_SETUP_DIR"
fi


if [ ! -e "$WORDPRESS_SETUP_DIR"/wp-addons.php ] ; then
    echo -e "$YELLOW"The wp-addons.php was not found in the project."$ENDCOLOR"
    if [ ! -e "$WORDPRESS_SETUP_DIR"/wp-addons.php.example ]; then
        echo -e "$YELLOW"Copying example wp-addons from hooks repo."$ENDCOLOR"
        cp "$CURRENT_DIR"/wp-addons.php.example "$WORDPRESS_SETUP_DIR"/wp-addons.php
        cp "$CURRENT_DIR"/wp-addons.php.example "$WORDPRESS_SETUP_DIR"
    else
        echo -e "$YELLOW"Copying example wp-addons from project repo."$ENDCOLOR"
        cp "$WORDPRESS_SETUP_DIR"/wp-addons.php.example "$WORDPRESS_SETUP_DIR"/wp-addons.php
    fi
...
if [ ! -e "$WORDPRESS_SETUP_DIR"/cleanup.sh ] ; then
    ...
    if [ ! -e "$WORDPRESS_SETUP_DIR"/cleanup.sh.example ]; then
        ...
        cp "$CURRENT_DIR"/cleanup.sh.example "$WORDPRESS_SETUP_DIR"/cleanup.sh
        cp "$CURRENT_DIR"/cleanup.sh.example "$WORDPRESS_SETUP_DIR"
    else
        echo -e "$YELLOW"Copying example cleanup.sh from project repo."$ENDCOLOR"
        cp "$WORDPRESS_SETUP_DIR"/cleanup.sh.example "$WORDPRESS_SETUP_DIR"/cleanup.sh
    fi
